...
# Insert private key as a hex string e.g. "112233aabbcc"
PRIVATE_KEY = ""
...
if __name__ == '__main__':
    # test_vectors()
    if len(sys.argv) > 1:
        hash_to_sign = sys.argv[1]
    else:
        hash_to_sign = input('Insert hash to be signed (hex encoded): ').strip()
    if PRIVATE_KEY == '':
        private_key = input('Insert private key (hex encoded): ').strip()
    else:
        private_key = PRIVATE_KEY

    sign = schnorr_sign(
        bytes.fromhex(hash_to_sign),
